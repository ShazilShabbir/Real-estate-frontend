...
export default function AgentDashboardPage() {
  ...
  const cards = [
    { label: "Total Properties", value: stats?.totalProperties || 0, icon: Building2, color: "text-blue-500 bg-blue-500/10" },
    { label: "Total Views", value: stats?.totalViews || 0, icon: Eye, color: "text-emerald-500 bg-emerald-500/10" },
    { label: "Total Inquiries", value: stats?.totalInquiries || 0, icon: MessageSquare, color: "text-purple-500 bg-purple-500/10" },
    { label: "Featured", value: stats?.featured || 0, icon: Star, color: "text-amber-500 bg-amber-500/10" },
    { label: "Approved", value: stats?.approved || 0, icon: CheckCircle2, color: "text-green-500 bg-green-500/10" },
    { label: "Pending Approval", value: stats?.pendingApproval || 0, icon: Clock, color: "text-orange-500 bg-orange-500/10" },
  ]
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <h2 className="text-lg font-bold text-foreground mb-4">Recent Properties</h2>
        {stats?.recentProperties?.length === 0 ? (
          <p className="text-sm text-muted-foreground py-8 text-center">No properties yet. Click "Add Property" above.</p>
        ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/60">
                    ...
                    <th className="text-left text-xs font-medium text-muted-foreground uppercase px-4 py-3">Price</th>
                    <th className="text-left text-xs font-medium text-muted-foreground uppercase px-4 py-3">Status</th>
                    <th className="text-center text-xs font-medium text-muted-foreground uppercase px-4 py-3">Featured</th>
                    <th className="text-center text-xs font-medium text-muted-foreground uppercase px-4 py-3">Views</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border/40">
                  {stats?.recentProperties.map((prop) => (
                    <tr key={prop._id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3"><span className="text-sm text-foreground">${prop.price?.toLocaleString()}</span></td>
                      <td className="px-4 py-3">
